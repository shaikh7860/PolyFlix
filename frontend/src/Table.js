    ...
    const rows = props.movieData.map((row, index) => {
        return (
          <tr key={index}>
              ...
              <td>
                <button onClick={() => navigate("/movie/" + row.title, {state: {id: row.id, title: row.title, vote_average: row.vote_average, poster_path: row.poster_path, overview: row.overview}}) }>View Info</button>
              </td>
            </tr>
          );
        });
